...
export default function VaultLayout({ children, vaultName }) {
  ...
  const [showMultiPartyModal, setShowMultiPartyModal] = useState(false);
  const [nominees, setNominees] = useState([
    { name: "", phone: "", isOwner: true }
  ]);
  const [requiredApprovals, setRequiredApprovals] = useState(2);
  const [nomineeName, setNomineeName] = useState("");
  const [nomineePhone, setNomineePhone] = useState("");
  const [isSelfNominee, setIsSelfNominee] = useState(false);
  const [showInactivityModal, setShowInactivityModal] = useState(false);


  const checkMultiPartySetup = async () => {
    try {
      ...
      console.error("Nominee check failed:", error);
      return false;
    }
  };
  ...
  const handleEncrypt = async (file) => {
    ...
    const passphrase = prompt("Enter a passphrase to encrypt this file:");
  ...
  return (
    <div style={styles.page}>
      ...
      {showVaultSecurityModal && (
        ...
        >
          <div style={styles.vaultModal} onClick={(e) => e.stopPropagation()}>
            ...
            <button 
              style={styles.vaultOptionBtn} 
              onClick={() => {
                setShowVaultSecurityModal(false);
                setShowMultiPartyModal(true);
              }}>
              Multi-Party Unlock
            </button>
            <button
              style={styles.vaultOptionBtn}
              onClick={ async () => {
                const isMultiPartyEnabled = await checkMultiPartySetup();

                if (!isMultiPartyEnabled) {
                  alert(
                    "Multi-Party Unlock is not configured.\n\nPlease add at least one nominee before enabling the Inactivity System."
                  );
                  return;
                }
      ...
      {/* Multi-Party Unlock Modal */}
      {showMultiPartyModal && (
        <div
          style={styles.modalOverlay}
          onClick={() => setShowMultiPartyModal(false)}
        >
          <div
            style={styles.vaultModal}
            onClick={(e) => e.stopPropagation()}
          >
            <h3 style={{ color: "#e50914" }}>
              Add Nominees for Vault Approval
            </h3>

            <p style={{ fontSize: "13px", color: "#aaa", textAlign: "center" }}>
              Select trusted people whose approval will be required to unlock this vault.
            </p>

            {nominees.map((n, idx) => (
              <div key={idx} style={{ width: "100%", display: "flex", gap: "8px" }}>
                <input
                  placeholder="Name"
                  value={n.name}
                  onChange={(e) => {
            ...
            <button
              ...
              onClick={() =>
                setNominees([...nominees, { name: "", phone: "", isOwner: false }])
              }
            >
              + Add Nominee
            </button>
            ...
            <button
              ...
              onClick={async () => {
                try {
                  // 1️⃣ Save vault multiparty settings
                  await pb.collection("vaults").update(vaultId, {
                  ...
                  // 2️⃣ Save nominees
                  await pb.collection("vault_nominees").create({
                    vault_id: vaultId,
                    nominees: JSON.stringify(nominees),
                  });

                  // 3️⃣ Send SMS to all nominees except the owner
                  nominees.forEach(n => {
                    if (!n.isOwner && n.phone.trim()) {
                      if (!isValidPhoneNumber(n.phone)) {
                        console.error("Invalid phone number:", n.phone);
                        return;
                      }
                  ...
                  setModalMessage("✅ Nominees set successfully and SMS sent!");
                  setShowMultiPartyModal(false);
                } catch (err) {
                  console.error("Failed to set nominees:", err);
                  setModalMessage("❌ Failed to set nominees. Check console.");
                }
              }}
            >
              Set Nominees
      ...
      {/* Inactivity System Security Modal */}
      {showInactivityModal && (
        <div
          style={styles.modalOverlay}
        ...
        >
          <div
            style={styles.geoModal}
            onClick={(e) => e.stopPropagation()}
          >
            <h3 style={{ color: "#e50914", marginBottom: "8px" }}>
              Inactivity System
            </h3>

            <p style={{ fontSize: "13px", color: "#aaa", textAlign: "center" }}>
              If the owner becomes inactive for a defined period, vault ownership
              will automatically transfer to one of the nominees.
            </p>
            ...
            <input
              type="date"
              style={styles.inputField}
              placeholder="Inactivity Start Date"
            />
            ...
            <p style={{ fontSize: "12px", color: "#888", textAlign: "center" }}>
              Ownership transfer is irreversible and will occur automatically
              after expiry.
            </p>

            <div style={{ display: "flex", gap: "10px", marginTop: "16px" }}>
              <button style={styles.saveBtn}>
                Enable Inactivity System
              </button>
              <button
